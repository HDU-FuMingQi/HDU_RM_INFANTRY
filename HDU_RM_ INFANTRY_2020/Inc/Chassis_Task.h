...
void CHASSIS_REST(void);  ////底盘启动状态/
/*****************键盘模式*************************/
void CHASSIS_InitArgument(void);  //键盘底盘参数初始化//
void CHAS_Key_Ctrl(void);    ////键盘控制底盘移动/

void Chassis_NORMAL_Mode_Ctrl(void);   ////底盘键盘模式选择,按键响应//
void Chassis_Mouse_Move_Calculate( int16_t sRevolMax );   /////鼠标控制底盘旋转,键盘QEC控制快速转圈//
...
/**************各种模式函数********************/
void CHASSIS_CORGI_Mode_Ctrl(int16_t sRevolMax, int16_t sRevolRamp);  //////扭屁股模式(位置不变版)/
void CHASSIS_SZUPUP_Mode_Ctrl(void);   ///手动爬坡模式/
void CHASSIS_MISS_Mode_Ctrl(void);    ////自动闪避模式/
void CHASSIS_PISA_Mode_Ctrl(void);   /////45°模式/
void CHASSIS_GYROSCOPE_Mode_Ctrl(int16_t sMoveMax,int16_t sMoveRamp);  ///小陀螺模式/

/*******************是否*******************/
bool CHASSIS_IfActiveMode(void); 	 ////获取底盘移动模式
bool Chassis_IfSZUPUP(void);       /////底盘是否处于爬坡模式
bool Chassis_IfCORGI(void);       //////底盘是否处于扭屁股模式
bool Chassis_IfPISA(void);       ////底盘是否处于45°模式
